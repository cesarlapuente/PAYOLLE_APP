package eu.randomobile.payolle.apppayolle.mod_global.environment;
...
import eu.randomobile.payolle.apppayolle.mod_global.data_access.DBManager;
...
public class ExternalStorage {
	...
	/* M�todo que comprueba la estructura de carpetas 
	...
	public static void comprobarDirectorioAppSD(Application application){
		...
		if(directorioAppSD.exists()){
			...
			// Establecemos la ruta inicial en la ra�z de la tarjeta
			File rutaRaiz = new File(Environment.getExternalStorageDirectory() + "/" + app.CARPETA_SD + "/");
			Log.d("rutaRaiz","contenu rutaRaiz "+rutaRaiz.getName() + " list Files"+ rutaRaiz.listFiles());
			// Guardamos los archivos que hay dentro de la ruta
			Log.d("rutaRaiz","ruta raiz list : "+rutaRaiz.list());

			String[] archivos = rutaRaiz.list();

			Log.d("rutaRaiz","archivos : "+archivos);
			//Log.d("rutaRaiz : ", "AbsolutePath " + rutaRaiz.getAbsolutePath() + " TotalSpace : " + rutaRaiz.getTotalSpace() + "rutaRaiz.lastModified()");
	...
	private static boolean actualizarBDExistente(Application application,String nombreBBDD) {
		...
		// Aqu� hay que comprobar las versiones de la BBDD y si son distintas hay que actualizar con la que va en
		// el paquete, que es la m�s actualizada
		...
		// Paso 1 renombrar la BBDD que ya hab�a en la SD
